class Spaceship {
  ...
  draw(){
    ...
    push();
        rotate(radians(180));
        image(img, - this.location.x, - this.location.y - 50, img.width / 20, img.height / 20);
        image(img, - this.location.x - 27, - this.location.y - 50, img.width / 20, img.height / 20);
    pop();
}
  ...
  setNearEarth(){
    //YOUR CODE HERE (6 lines approx)
      ...
      friction.mult(-1);
      friction.normalize();
      friction.mult(this.velocity.y / 30);
